...
import type { ChatSession, SendChatMessagePayload, SendChatMessageResponse } from "@/types/chat";
...
export const chatApi = {
  ...
  getMessages: async (sessionId: string) => {
    const response = await apiClient.get<ApiResponse<any[]>>(`/chat/sessions/${sessionId}/messages`);
    return response.data;
  },

  sendMessage: async (sessionId: string, payload: SendChatMessagePayload) => {
  ...
  archiveSession: async (sessionId: string) => {
    const response = await apiClient.patch<ApiResponse<null>>(`/chat/sessions/${sessionId}/archive`);
    return response.data;
  },
